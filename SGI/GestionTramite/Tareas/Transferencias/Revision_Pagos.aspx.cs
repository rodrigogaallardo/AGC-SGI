...
namespace SGI.GestionTramite.Tareas.Transferencias
{
    public partial class Revision_Pagos : System.Web.UI.Page
    {
        ...
        public void cargarBoletas(int id_solicitud)
        {
            ...
            {
                var lstPagos = (from p in db.SGI_Solicitudes_Pagos
                                ...
                                select new clsItemGrilla
                                {
                                    id_pago = p.id_pago,
                                    monto_pago = p.monto_pago,
                                    nro_boleta_unica = p.nro_boleta_unica,
                                    CreateDate = p.CreateDate,
                                }
                                ).ToList();


                var lstPagosTransf = (from solpag in db.Transf_Solicitudes_Pagos
                                      where solpag.id_solicitud == id_solicitud
                                      select new clsItemGrilla
                                      {
                                          id_pago = solpag.id_pago,
                                          monto_pago = solpag.monto_pago,
                                          nro_boleta_unica = solpag.id_solicitud,
                                          CreateDate = solpag.CreateDate
                                      }).ToList();

                lstPagos.AddRange(lstPagosTransf);
        ...
        internal class clsItemGrilla
        {
            public int id_pago { get; set; }
            public decimal monto_pago { get; set; }
            public long? nro_boleta_unica { get; set; }
            ...
            try
            {
                ...
                using (TransactionScope Tran = new TransactionScope())
                {
                    ...
                    try
                    {
                        ...
                        this.db.SGI_Transf_ActualizarEstado(this.id_solicitud, (int)Constants.Solicitud_Estados.En_trámite, userid);
