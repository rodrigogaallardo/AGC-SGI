...
namespace SGI.GestionTramite.Tareas.Dictamen
{
    public partial class Realizar_Dictamen : System.Web.UI.Page
    {
        ...
        private void CargarDatosTramite(int id_tramitetarea)
        {
            ...
            if (tramite_tarea.id_tarea == (int)Constants.ENG_Tareas.ESCU_HP_Dictamen_Realizar ||
                tramite_tarea.id_tarea == (int)Constants.ENG_Tareas.ESCU_IP_Dictamen_Realizar)
            {
        ...
        protected void ucResultadoTarea_FinalizarTareaClick(object sender, ucResultadoTareaEventsArgs e)
        {
            try
            {
                ...
                if (this.id_tarea != (int)Constants.ENG_Tareas.ESCU_HP_Dictamen_Realizar &&
                    this.id_tarea != (int)Constants.ENG_Tareas.ESCU_IP_Dictamen_Realizar)
                {
                    sinProceso = true;
                }
                if (!sinProceso && !hayProcesosGenerados)
                {
                    db.SGI_HAB_GenerarProcesos_SADE_v4_SinPase(this.TramiteTarea, userid);
