...
namespace SGI.GestionTramite.Tareas.CPadron
{
    public partial class Carga_Tramite : BasePage
    {
        ...
        private void CargarDatosTramite(int id_tramitetarea)
        {
            ...
            if (ttTR != null)
            {
                this.id_solicitud = db.Transf_Solicitudes.Where(x => x.id_solicitud == ttTR.id_solicitud).Select(y => y.id_cpadron).FirstOrDefault();
           ...
           // SGI_Tramites_Tareas_CPADRON ttCP = db.SGI_Tramites_Tareas_CPADRON.FirstOrDefault(x => x.id_tramitetarea == id_tramitetarea);
            ...
            ucDocumentoAdjunto.LoadData((int)Constants.GruposDeTramite.CP, this.id_solicitud, id_tramitetarea);
        ...
        private SGI_Tarea_Carga_Tramite Buscar_tarea_anterior(int id_cpadron)
        {
            SGI_Tarea_Carga_Tramite tarea =
                (
                    ...
                    join gere in db.SGI_Tarea_Carga_Tramite on tt_cp.id_tramitetarea equals gere.id_tramitetarea
                    where tt_cp.id_cpadron == id_cpadron
                    && tt.id_tarea == (int)Constants.ENG_Tareas.CP_Carga_Informacion
                    orderby gere.id_carga_tramite descending
                    select gere
                ).ToList().FirstOrDefault();
