<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ucSolicitudesPendientesCalificar.ascx.cs" Inherits="SGI.Dashboard.Controls.ucSolicitudesPendientesCalificar" %>
...
<script type="text/javascript">
    ...
    function cargar_ph_sol_pend() {
        ...
        var ws_url = "<%: ResolveUrl("~/Dashboard/Dashboard.aspx/getDatos_solicitudes_pendientes_calif") %>" 
        debugger;
        $.ajax({
            url: ws_url,
            type: "POST",
        ...
        sol_asignadas.bind("plotclick", function (event, pos, item) {
            ...
            //    var cant = item.series.data[0][1];
            //    html.push("<div style='border:1px solid grey;background-color:",
            //         item.series.color, ";text-align:left;'>",
            //         "<span style='color:black;'>",
            //         "Calificador: ", item.series.label,
            //         "</br>Solicitudes asignadas: ", cant,
            //         " (", Math.round(percent), "%)",
            //         "</span>",
            //         "</div>");
            //    $("#ph_sol_pend_memo").html(html.join(''));
